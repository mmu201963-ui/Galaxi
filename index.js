...
async function closeAllForProfitReset(reason = 'PORTFOLIO_PROFIT_5PCT') {
  ...
  const positionsBefore = Array.isArray(state.positions) ? state.positions.length : 0;
  if (!positionsBefore) {
    state.profitTargetBaseline = Number(state.equity || cfg.capital);
    updateProfitTarget();
    state.profitTargetHits = Number(state.profitTargetHits || 0) + 1;
    state.cycle = 0;
    state.lastSignal = `OBJETIVO +${cfg.portfolioProfitTargetPct.toFixed(2)}% · sin posiciones · NUEVO CICLO`;
    writeState();
    return true;
  }
  ...
  try {
    ...
    state.lastSignal = `OBJETIVO +${cfg.portfolioProfitTargetPct.toFixed(2)}% ALCANZADO · ${positionsBefore} posiciones cerradas · NUEVO CICLO`;
    pushHistory({
      action: 'PORTFOLIO_RESET',
      reason,
      equityBefore: round(before, 2),
      equityAfter: round(after, 2),
      closedPositions: positionsBefore,
      nextBaseline: round(state.profitTargetBaseline, 2),
      nextTarget: round(state.profitTargetEquity, 2)
    });
    writeState();
    return true;
  } finally {
